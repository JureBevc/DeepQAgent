...
class DeepQAgent():
    ...
    def learn(self):
        ...
        batch = np.random.choice(current_memory_size, self.batch_size, replace=False)
        batch_index = np.arange(self.batch_size, dtype=np.int32)
        
        state_batch = T.tensor(self.state_memory[batch]).to(self.model.device)
        new_state_batch = T.tensor(self.new_state_memory[batch]).to(self.model.device)
        reward_batch = T.tensor(self.reward_memory[batch]).to(self.model.device)
        terminal_batch = T.tensor(self.terminal_memory[batch]).to(self.model.device)
        action_batch = self.action_memory[batch]
        
        current_q = self.model.forward(state_batch)[batch_index, action_batch] # Select for update only those actions, that were taken
        ...
        loss = self.target_model.loss(target_q, current_q).to(self.model.device)
        loss.backward()
